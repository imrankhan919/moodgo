...
import { sendFromUserMessage, sendMessage } from "../features/chat/chatSlice"
...
function ChatWidget() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    dispatch(sendFromUserMessage({ sender: "user", message }))
      ...
      <details className="group">
        ...
        <div className="absolute bottom-20 right-0 w-[320px] h-[450px] bg-[#111118] border border-[#1F1F2E] rounded-2xl shadow-2xl overflow-hidden animate-[slideUp_0.3s_ease_forwards] flex flex-col">
          ...
          <div className="p-3 border-t border-[#1F1F2E]">
            <form onSubmit={handleSubmit} className="flex items-center gap-2 bg-[#0A0A0F] rounded-xl px-4 py-2.5 border border-[#1F1F2E] focus-within:border-[#4F8EF7]/50 transition-all duration-300">
              <input
                type="text"
                value={message}
